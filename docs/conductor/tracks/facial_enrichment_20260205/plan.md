...
### Phase 3: Integration and Storage [checkpoint: c36f210]
- [x] Task: Update `extractor/cli.py` to invoke `FacialEngine` during the enrichment loop. [verified]
- [x] Task: Update `image_metadata.json` saving logic to include the `faces` key. [verified]
- [x] Task: Implement TDD for integrated enrichment: Write failing tests in `tests/test_facial_integration.py` to verify full metadata output. [verified]
- [x] Task: Verify tests pass and ensure cases with zero faces are handled (empty list). [verified]
- [x] Task: Conductor - User Manual Verification 'Phase 3: Integration and Storage' (Protocol in workflow.md)
